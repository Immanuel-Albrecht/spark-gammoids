...
class Digraph[V](
  ...
  /**
    * iterate over all arcs as pairs
    *
    * @return  arc iterable
    */
  def iterateArcs(): Iterable[(V, V)] = {
    _incidence.toIterable
      .flatMap({ case (u, vs) ⇒ vs.map((u, _)) })
  }
  ...
  def isValid(failFast: Boolean): TestResult = {
    ...
    lazy val incident_vertices: Set[V] = _incidence
      ...
      .toSet ++
      _incidence.keySet ++ _invIncidence
      .flatMap({
        case (u, vs) ⇒
          vs
      })
    ...
    lazy val arcs_by_incidence: Set[(V, V)] = _incidence
      .flatMap({
        case (u, vs) ⇒
          vs.map({
            case v ⇒ (u, v)
    ...
    lazy val arcs_by_invIncidence: Set[(V, V)] = _invIncidence
      .flatMap({
        case (v, us) ⇒
          us.map({
            case u ⇒ (u, v)
    ...
    val tests: List[() ⇒ TestResult] =
      List(
        () ⇒ {
          if (incident_vertices subsetOf vertexSet)
            TestResult(
              true,
            ...
            TestResult(
              false,
              f"[x] ${incident_vertices.diff(vertexSet).size} referenced vertices are not in the vertex set of this digraph." :: Nil
            )
        },
        () ⇒ {
          if (arcs_by_incidence == arcs_by_invIncidence)
            TestResult(
              true,
            ...
            TestResult(
              false,
              f"[x] incidence has ${arcs_by_incidence.diff(arcs_by_invIncidence).size} extra arcs, invIncidence has ${arcs_by_invIncidence.diff(arcs_by_incidence).size} extra arcs" :: Nil
            )
          }
        }
      )
      ...
      .foldLeft((trivial, trivial))({
        case ((all_p, new_p), _) ⇒ {
          val longer_paths = new_p.flatMap({
            case (path, pathstats) ⇒ {
              _invIncidence
                .getOrElse(pathstats.source, Set())
                .flatMap(v0 ⇒ {
  ...
  override def paths(
    ...
    t.foldLeft(Set[QuasiPath[V]]())({
      case (paths, target) ⇒ {
        s.foldLeft(paths)({
          case (paths, source) ⇒
            paths ++ pathStatsEndingInStartingIn
              .getOrElse(target, Lazy(Map[V, Set[QuasiPath[V]]]()))()
              .getOrElse(source, Set[QuasiPath[V]]())
              .filter(ps ⇒ avoid.intersect(ps.visited).isEmpty)
              .toSet
        })
      }
    })
